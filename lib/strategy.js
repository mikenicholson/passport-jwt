 ...
 */
async function JwtStrategy(options, verify) {

    await passport.Strategy.call(this);
    this.name = 'jwt';
    ...
    if (options.secretOrKey) {
        ...
        this._secretOrKeyProvider = async function (request, rawJwtToken, done) {
            await done(null, options.secretOrKey)
        };
    }
    ...
    //on the options.
    this._verifOpts = await assign({}, jsonWebTokenOptions, {
...
JwtStrategy.prototype.authenticate = async function(req, options) {
    ...
    await this._secretOrKeyProvider(req, token, async function(secretOrKeyError, secretOrKey) {
        if (secretOrKeyError) {
            self.fail(secretOrKeyError)
        } else {
            // Verify the JWT
            await JwtStrategy.JwtVerifier(token, secretOrKey, self._verifOpts, async function(jwt_err, payload) {
                if (jwt_err) {
                    ...
                    try {
                        if (self._passReqToCallback) {
                            await self._verify(req, payload, verified);
                        } else {
                            await self._verify(payload, verified);
                        }
                    } catch(ex) {
                        await self.error(ex);
                    }
